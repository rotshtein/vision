from communication import PREAMBLE_PREFIX
...
class MessagesReceiverHandler(object):
    ...
    def handle_setup_msg(self, message):
        for rx_listener in self.rx_listeners:
            rx_listener.on_setup_message(HDSetupMessage.from_bytes(message))

    def handle_set_warning_msg(self, message):
        for rx_listener in self.rx_listeners:
            rx_listener.on_set_warning_msg(HDSetWarningMessage.from_bytes(message))

    def handle_remove_warning_msg(self, message):
        for rx_listener in self.rx_listeners:
            rx_listener.on_remove_warning_msg(message)

    def handle_remove_all_warnings_msg(self, message):
        for rx_listener in self.rx_listeners:
            rx_listener.on_remove_all_warnings_msg(message)

    def handle_remove_all_warnings_except_defaults_msg(self, message):
        for rx_listener in self.rx_listeners:
            rx_listener.on_remove_all_warnings_except_defaults_msg(message)

    def handle_set_warning_to_default_msg(self, message):
        for rx_listener in self.rx_listeners:
            rx_listener.on_set_warning_to_default_msg(message)

    def handle_set_power_msg(self, message):
        for rx_listener in self.rx_listeners:
            rx_listener.on_set_power_msg(message)

    def handle_get_warning_msg(self, message) -> HDGetWarningResponse:
    ...
    def handle_get_warning_config_msg(self, message):
        ...
        for rx_listener in self.rx_listeners:
            data = rx_listener.on_get_warning_config_msg(message)
            if data is not None:
                response = data
        return response

    def handle_get_setup_config_msg(self, message):
        # compose the responses from HD and from vision.
        # Both implement the same message but each will update only its relevant fields.
        # non relevant fields will be None!
        response = HDGetSetupConfigResponse()
        for rx_listener in self.rx_listeners:
            data = rx_listener.on_get_setup_config_msg(message)
    ...
    def handle_get_status_msg(self, message):
        response = HDGetStatusResponse()
        for rx_listener in self.rx_listeners:
            data = rx_listener.on_get_status_msg(message)
            if data is not None:
                response = data
        return response
    ...
    def build_response_message(self, opcode, data=None):
        ...
        total_msg_length = bytes(data_length + MSG_FIXED_LENGTH_WITHOUT_DATA)
        res_message_bytes_array = PREAMBLE_PREFIX + total_msg_length + opcode + data
