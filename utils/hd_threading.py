...
class HDThread(threading.Thread, IRXMessage):
    ...
    def run(self) -> None:
        while not self.is_exit:
            ...
            if self.fps != 0 and self.iteration_time_sec != 0:
                ...
                self.logging.debug("{} - Going to sleep {0:.2f} sec".format(self.thread_name, sleep_time))
                sleep(sleep_time)
    ...
    def _calc_fps(self):
        last_measured_fps = 1.0 / (datetime.now() - self.last_measured_time).microseconds * 1000000
        self.logging.info("{} - last measured fps={}".format(self.thread_name, last_measured_fps))
        self.last_measured_time = datetime.now()
